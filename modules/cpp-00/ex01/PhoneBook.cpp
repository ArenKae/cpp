...
/*   Updated: 2024/06/17 14:50:40 by acosi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_stoi(std::string input, int *num)
{
	...
	if (ss.fail()) {
		if (input == "")
			print_error("Index must be a numerical value.", NONEWLINE);
		else
			print_error("Index must be a numerical value.", NEWLINE);
		return 1;}
	else if (*num < 0 || *num > 8) {
		print_error("Index must be between 0 and 8.", NEWLINE);
		return 1;}
	return 0;
}
